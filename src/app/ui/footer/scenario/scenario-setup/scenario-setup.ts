...
export class ScenarioSetupComponent implements OnInit {
  ...
  updateMonster() {
    ...
    if (
      !sections &&
      gameManager.scenarioManager.getMonsters(this.scenario, false, this.scenario.custom).length !=
        gameManager.scenarioManager.getMonsters(this.scenario, true, this.scenario.custom).length
    ) {
    ...
    gameManager.scenarioManager.getMonsters(this.scenario, sections, this.scenario.custom).forEach((monsterData) => {
      const monster: Monster = new Monster(monsterData, gameManager.game.level);
      if (
        this.spoiler ||
      ...
      let statEffectRules = this.scenario.rules
        ? this.scenario.rules.filter((rule) => rule.statEffects).flatMap((rule) => rule.statEffects)
        : [];
      let statEffectRule: StatEffectRule | undefined = statEffectRules.find(
        (statEffectRule) => gameManager.figuresByIdentifier(statEffectRule.identifier, false, [monster]).length
      );
      ...
      if (!statEffectRule) {
        gameManager.scenarioManager.getSections(this.scenario).forEach((sectionData) => {
          if (!statEffectRule && sectionData.rules) {
            ...
            statEffectRule = statEffectRules.find(
              (statEffectRule) => gameManager.figuresByIdentifier(statEffectRule.identifier, false, [monster]).length
            );
          }
        });
