...
export class EventCardDeckComponent {
  ...
  dropUpcoming(event: CdkDragDrop<EventCard[]>) {
    ...
    if (event.container === event.previousContainer) {
      ...
      (gameManager.game.party.eventDecks[this.type] || []).splice(
        ...
        0,
        this.newCards[event.previousIndex].cardId
      );
    }
    gameManager.stateManager.after();
  }
  ...
  dropDrawn(event: CdkDragDrop<EventCard[]>) {
    ...
    if (eventCard) {
      ...
      gameManager.game.party.eventCards.splice(
        event.currentIndex,
        0,
        new EventCardIdentifier(eventCard.cardId, eventCard.edition, eventCard.type, -1, [], [], false, false)
      );
      gameManager.stateManager.after();
    }
  }
