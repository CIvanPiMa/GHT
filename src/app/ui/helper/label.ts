...
import { ActionType, ActionTypesIcons } from 'src/app/game/model/data/Action';
import {
  ...
  while (value.match(ghsLabelRegex)) {
    value = value.replace(ghsLabelRegex, (match, ...args) => {
      ...
      if (type === 'condition' || type === 'immunity') {
        ...
        image =
          '<img  src="./assets/images/' + (fh ? 'fh/' : '') + split.join('/') + '.svg" class="icon' + (ghsSvg ? ' ght-svg' : '') + '">';
        replace =
        ...
        replace =
          '<span class="attack-modifier-effect placeholder-element-half element-half-placeholder' +
          (fh ? ' fh' : '') +
          '"><span class="element-half-container' +
          (consume ? ' consume' : '') +
          '"><span class="element-half"><img src="./assets/images/' +
          (fh ? 'fh/' : '') +
          'element/' +
          elements[0] +
        ...
        value.split('|').forEach((hexValue) => {
          ...
          if (hex !== null) {
            replace +=
              ...
              ';grid-row-end:' +
              (hex.y + 1) +
              '"><img src="./assets/images/action/hex/' +
              hex.type +
              '.svg"></span>';
          }
        });
        ...
        image = '<img class="ght-svg" src="./assets/images/initiative.svg"></span>';
        replace = '<span class="placeholder-initiative">' + split[2] + image + '</span>';
      } else if (type === 'action' && split.length === 4) {
        image = '<img  src="./assets/images/action/' + split[2] + '/' + split[3] + '.svg" class="icon ght-svg">';
        replace = '<span class="placeholder-perk">' + image + value + '</span>';
      } else if (prefix === 'game' && type === 'items' && split.length === 4) {
        image = '<img  src="./assets/images/items/' + split[2] + '/' + split[3] + '.svg" class="icon ght-svg">';
        ...
        if (split[2] === 'random') {
          ...
          replace =
            '<span class="placeholder-item-fh">' +
            '<span class="value">' +
            (itemId < 100 ? '0' : '') +
            (itemId < 10 ? '0' : '') +
            itemId +
        ...
        image = '<img class="icon ght-svg" src="./assets/images/action/card/' + card + '.svg">';
        ...
        if (townGuardAM) {
          ...
          let amImage =
            ...
            '.png" class="icon"><span class="value">' +
            (townGuardAM.value >= 0 ? '+' : '') +
            townGuardAM.value +
            '</span></span>';
        ...
        const effectImage = '<img  src="./assets/images/action/custom/' + value + '.svg" class="icon ght-svg">';
        replace = '<span class="placeholder-custom-action">' + effectImage + '</span>';
      } else if (type === 'experience') {
        image = '<img  src="./assets/images/experience.svg" class="icon ght-svg">';
        ...
        image = '<img class="icon" src="' + gameManager.characterManager.characterIcon(characterName) + '">';
        replace =
          '<span class="placeholder-character-icon-colored-bg" style="background-color:' +
          gameManager.characterManager.characterColor(characterName) +
          '">' +
          image +
          '</span>';
        ...
        image = '<img src="' + icon + '">';
        replace =
          '<span class="placeholder-character-token' +
          (additionalToken ? ' additional' : '') +
          '" style="--ght-character-color:' +
          gameManager.characterManager.characterColor(characterName) +
          '">' +
          image +
          '</span>';
        ...
        image = '<img src="' + icon + '">';
        replace = '<span class="placeholder-character-token" style="--ght-character-color:#' + color + '">' + image + '</span>';
        ...
        image = '<img  src="./assets/images/fh/action/target.svg" class="icon ght-svg">';
        ...
        image = '<img  src="./assets/images/fh/action/damage.svg" class="icon ght-svg">';
        ...
        image = '<img  src="./assets/images/fh/loot/' + split[2] + '.svg" class="icon ght-svg">';
        replace = '<span class="placeholder-resource">' + image + '</span>';
      } else if (type === 'section' && value) {
        image = '<img src="./assets/images/fh/party/section.svg" class="icon ght-svg">';
        replace = '<span class="placeholder-section">' + image + value + '</span>';
      } else if (type === 'gh2eFaction' && value) {
        image = '<img src="./assets/images/gh2e/faction/' + value + '.svg" class="icon ght-svg">';
        replace = '<span class="placeholder-faction">' + image + '</span>';
      } else if (type === 'checkmark') {
        replace = '<span class="placeholder-checkmark"><img src="./assets/images/check.svg" class="icon ght-svg"></span>';
      } else if (type === 'prosperity') {
        replace = '<span class="placeholder-prosperity"><img src="./assets/images/gh2e/prosperity.svg" class="icon ght-svg"></span>';
      } else if (type === 'itemSlot' && value) {
        replace =
          '<span class="placeholder-items-slot"><img src="./assets/images/items/slots/' + value + '.svg" class="icon ght-svg"></span>';
      } else if (type === 'bbAm' && split.length === 3) {
        replace =
          '<span class="placeholder-bb-am"><img src="./assets/images/bb/attackmodifier/' + split[2] + '.svg" class="icon ght-svg"></span>';
        ...
        image = '<img src="./assets/images/fh/icons/' + value + '.svg" class="icon ght-svg">';
        replace = '<span class="placeholder-fh-icon">' + image + '</span>';
      } else if (type === 'gameIcon' && value) {
        image = '<img src="./assets/images/' + value + '.svg" class="icon ght-svg">';
        ...
        image = '<img src="./assets/images/fh/character/traits/trait.svg" class="icon ght-svg">';
        replace =
        ...
        replace = '<span class="placeholder-action"><img src="./assets/images/eventcards/' + value + '.svg" class="icon ght-svg"></span>';
        ...
        let labelArgs = label
          .split(':')
          .splice(1)
          .map((arg) => applyPlaceholder(settingsManager.getLabel(arg), placeholder, relative));
        if (value) {
...
export class GhsLabelDirective implements OnInit, OnChanges {
  ...
  readonly value = input.required<string | number>({ alias: 'ght-label' });
  readonly args = input<(string | number | boolean)[]>([], { alias: 'ght-label-args' });
  readonly argLabel = input<boolean>(true, { alias: 'ght-label-args-replace' });
  readonly empty = input<boolean>(true, { alias: 'ght-label-empty' });
  readonly attribute = input<string>('', { alias: 'ght-label-attribute' });
...
@Directive({
  selector: '[ght-label-element]'
})
export class GhsLabelElementDirective implements OnInit {
  private el = inject(ElementRef);

  value: string = '';
  readonly prefix = input<string>('', { alias: 'ght-label-element' });
