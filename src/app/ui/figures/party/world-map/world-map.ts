...
@Component({
  imports: [NgClass, FormsModule, GhsLabelDirective, GhsTooltipDirective],
  selector: 'ght-world-map',
...
export class WorldMapComponent implements AfterViewInit {
  ...
  update() {
    ...
    if (editionData && editionData.worldMap) {
      ...
      if (this.worldMap) {
        ...
        gameManager
          .sectionData(this.edition)
          .filter((sectionData) => sectionData.conclusion && sectionData.coordinates && sectionData.unlocks && sectionData.unlocks.length)
          .forEach((sectionData) => {
            const visible =
        ...
        if (editionData.campaign && editionData.campaign.buildings) {
          this.buildings = editionData.campaign.buildings.filter(
            (buildingData) => buildingData.coordinates && buildingData.coordinates.length
          );
        }
      }
    }
  ...
  async initMap() {
    ...
    if (this.worldMap) {
      ...
      this.scenarios.forEach((scenarioData, i) => {
        if (scenarioData.coordinates) {
          ...
          if (!this.data.pick) {
            overlay.on('click', () => {
              if (
                !settingsManager.settings.debugEditWorldMap &&
                (!gameManager.stateManager.permissions || gameManager.stateManager.permissions.scenario)
              ) {
      ...
      if (this.data.pick) {
        this.map.on('click', (e) => {
          const coordinate = new WorldMapCoordinates();
  ...
  placeOverlay(url: string, coordinates: WorldMapCoordinates, height: number, index: number): ImageOverlay {
    const overlay = this.L.imageOverlay(
      url,
      [
        [height - coordinates.y, coordinates.x],
        [height - coordinates.y - coordinates.height, coordinates.x + coordinates.width]
      ],
      { interactive: true }
    );
    ...
    if (settingsManager.settings.debugEditWorldMap) {
      ...
      if (element && element.getAttribute('drag')) {
        ...
        if (!!this.worldMap) {
          console.debug(
            '"x": ' +
              target.getBounds().getSouthWest().lng +
              ',\n' +
              '"y": ' +
              (this.worldMap.height - target.getBounds().getNorthEast().lat) +
              ','
          );
        }
      ...
      if (element && element.getAttribute('drag')) {
        ...
        if (lat && lng) {
          ...
          const bounds = this.L.latLngBounds(
            [target.getBounds().getSouthWest().lat + lat, target.getBounds().getSouthWest().lng + lng],
            [target.getBounds().getNorthEast().lat + lat, target.getBounds().getNorthEast().lng + lng]
          );
