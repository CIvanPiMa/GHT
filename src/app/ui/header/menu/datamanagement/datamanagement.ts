...
@Component({
  ...
  selector: 'ght-datamanagement-menu',
...
export class DatamanagementMenuComponent implements OnInit {
  ...
  async exportGame() {
    try {
      ...
      if (gameModel) {
        ...
        downloadButton.setAttribute('download', 'ght-game' + (gameModel.party.name ? '_' + gameModel.party.name : '') + '.json');
    ...
    try {
      ...
      if (backups && backups.length > 0) {
        ...
        downloadButton.setAttribute(
          'download',
          'ght-game' + (backup.party.name ? '_' + backup.party.name : '') + '-rev' + backup.revision + '.json'
        );
        document.body.appendChild(downloadButton);
        downloadButton.click();
        document.body.removeChild(downloadButton);
  ...
  importFile(inputEvent: any) {
    ...
    try {
      reader.addEventListener('load', async (event: any) => {
        ...
        } else if ((data.game && data.settings) || (typeof data === 'object' && data['ght-game'] && data['ght-settings'])) {
  ...
  async exportSettings() {
    try {
      ...
      if (settings) {
        ...
        downloadButton.setAttribute('download', 'ght-settings.json');
        document.body.appendChild(downloadButton);
        downloadButton.click();
        document.body.removeChild(downloadButton);
      }
  ...
  async exportDataDump() {
    await gameManager.stateManager.autoBackup('ght-data-dump-' + new Date().toISOString() + '.json', true);
  }
  ...
  async importDataDump(data: any) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      const key = origKey.startsWith('ght-') ? origKey.replace('ght-', '') : origKey;
      if (key === 'game') {
