...
export class CharacterItemsComponent implements OnInit {
  ...
  itemChange(itemIndexChange: number = 0) {
    setTimeout(() => {
      if (itemIndexChange !== 0) {
        ...
        this.item = gameManager.itemManager.getItem(
          this.itemIndex,
          (this.itemEdition && this.itemEdition) || this.character.edition,
          false
        );
  ...
  canBuy(item: ItemData | undefined, cost: number = 0): boolean {
    if (item) {
      return (
        (item.cost && item.cost + gameManager.itemManager.pricerModifier() + cost <= this.character.progress.gold && this.canAdd(item)) ||
        false
      );
    }
    return false;
  ...
  toggleEquippedItem(itemData: ItemData, force: boolean = false) {
    ...
    if (
      ...
      gameManager.stateManager.before(
        gameManager.itemManager.isEquipped(itemData, this.character) ? 'unequipItem' : 'equipItem',
        gameManager.characterManager.characterName(this.character, true, true),
        itemData.id,
        itemData.edition
      );
