...
@Component({
  imports: [NgClass, GhsLabelDirective, GhsTooltipDirective, PointerInputDirective, TrackUUIDPipe, CharacterItemComponent],
  selector: 'ght-character-item-list',
...
export class CharacterItemListComponent implements OnInit {
  ...
  update() {
    ...
    this.items = (
      gameManager.bbRules()
        ...
        : this.character.progress.items.map((identifier) => gameManager.itemManager.getItem(identifier.name, identifier.edition, true))
    )
      .filter(
        (itemData) => itemData && (this.setup || !settingsManager.settings.characterItemsPermanentEquipped || this.equipped(itemData))
      )
  ...
  zoom(value: number) {
    ...
    if (settingsManager.settings.animations) {
      ...
      }, 500 * settingsManager.settings.animationSpeed);
    }
  }
