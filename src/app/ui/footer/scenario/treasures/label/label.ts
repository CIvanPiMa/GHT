...
@Component({
  imports: [GhsLabelDirective, TrackUUIDPipe, ItemComponent],
  selector: 'ght-treasure-label',
      ...
      if (this.itemCards) {
        this.treasure.rewards.forEach((reward, index) => {
          if (
            ...
            [TreasureRewardType.randomItem, TreasureRewardType.randomItemBlueprint, TreasureRewardType.randomItemDesign].includes(
              reward.type
            ) &&
            this.rewardResults &&
            this.rewardResults[index] &&
            this.rewardResults[index][0] &&
            !isNaN(+this.rewardResults[index][0])
          ) {
    ...
    switch (reward.type) {
      ...
      case TreasureRewardType.itemBlueprint:
        ...
        value.split('+').forEach((itemString) => {
          ...
          if (item) {
            ...
            itemNameValues.push('<img class="icon ght-svg" src="./assets/images/warning.svg"> %item% ' + itemId);
          }
        });
      ...
      case TreasureRewardType.scenario:
        ...
        if (scenarioData) {
          ...
          return [this.labelPrefix + reward.type, value, '<img class="icon ght-svg" src="./assets/images/warning.svg"> %scenario%'];
        }
      ...
      case TreasureRewardType.resource:
        const resources = value
          ...
          .map(
            (resource) =>
              resource.split('-')[1] + '<img class="icon ght-svg" src="./assets/images/fh/loot/' + resource.split('-')[0] + '.svg">'
          );
