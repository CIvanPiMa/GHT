...
export class SettingsMenuComponent {
  ...
  zoom(value: number, force: boolean) {
    let factor: number = +window.getComputedStyle(document.body).getPropertyValue('--ght-factor');
    factor += value;
  ...
  setZoom(zoom: number, value: number, force: boolean) {
    if (settingsManager.settings.zoom !== zoom) {
      document.body.style.setProperty('--ght-factor', zoom + '');
      if (!force) {
