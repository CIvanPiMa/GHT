...
@Component({
  ...
  selector: 'ght-character-move-resources',
...
export class CharacterMoveResourcesDialog implements OnInit {
  ...
  changeLoot(lootType: LootType, value: number) {
    ...
    if ((this.loot[lootType] as number) < 0) {
      ...
      !this.character.progress.loot[lootType] ||
      (this.loot[lootType] as number) > (this.character.progress.loot[lootType] as number)
    ) {
