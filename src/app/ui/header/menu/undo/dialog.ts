...
import { InViewportDirective } from 'src/app/ui/helper/in-viewport';
...
export class UndoDialogComponent implements OnInit {
      ...
      this.undoOffset =
        gameManager.stateManager.undos.length > 0
          ? gameManager.game.revision -
            ...
            this.getUndoRevision(gameManager.stateManager.undos.length - 1) -
            1
          : 0;
      this.undoArray = Array.from({ length: Math.min(10, gameManager.stateManager.undos.length) }).map(
        (value, i) => gameManager.stateManager.undos.length - i - 1
      );
      this.redoArray = Array.from({ length: Math.min(10, gameManager.stateManager.redos.length) }).map(
        (value, i) => Math.min(10, gameManager.stateManager.redos.length) - i - 1
      );
    });
  ...
  moreUndos() {
    if (this.undoArray.length < gameManager.stateManager.undos.length) {
      this.undoArray = Array.from({ length: Math.min(gameManager.stateManager.undos.length, this.undoArray.length + 10) }).map(
        (value, i) => gameManager.stateManager.undos.length - i - 1
      );
    }
  }
  ...
  getUndoInfo(index: number): string[] {
    ...
    if (
      gameManager.stateManager.undos.length > 0 &&
      gameManager.stateManager.undoInfos.length >= gameManager.stateManager.undos.length &&
      index >= 0 &&
  ...
  getRedoInfo(index: number): string[] {
    ...
    if (
      ...
      gameManager.stateManager.undoInfos.length >= gameManager.stateManager.undos.length + gameManager.stateManager.redos.length &&
      index >= 0 &&
