...
export class SettingsManager {
  ...
  async loadSettings() {
    ...
    if (loadDefault) {
      try {
        await fetch('./ght-settings-default.json')
          .then((response) => {
            if (!response.ok) {
              throw Error();
            }
  ...
  async apply(setting: string, value: any) {
    if (setting === 'fullscreen') {
      ...
      document.body.style.setProperty('--ght-animation-speed', value + '');
    } else if (setting === 'fontsize') {
      document.body.style.setProperty('--ght-fontsize', value + '');
    } else if (setting === 'barsize') {
      document.body.style.setProperty('--ght-barsize', value + '');
    } else if (setting === 'globalFontsize') {
      document.body.style.setProperty('--ght-global-fontsize', value + '');
