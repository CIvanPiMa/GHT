...
@Component({
  ...
  selector: 'ght-character-battlegoals',
...
export class CharacterBattleGoalsDialog {
      ...
      this.dialog
        ...
        .closed.subscribe({
          next: () => {
            ...
            if (this.redrawAvailable > (gameManager.fhRules(true) || settingsManager.settings.battleGoalsFh ? 2 : 1) && this.data.draw) {
              this.drawCards();
            } else {
              this.close();
            }
  ...
  update() {
    this.available = gameManager.battleGoalManager.getUnrevealedBattleGoals().length;
    this.redrawAvailable = gameManager.battleGoalManager.getUnrevealedBattleGoals(this.character).length;
    this.battleGoals = this.character.battleGoals
      .map((identifier) => gameManager.battleGoalManager.getBattleGoal(identifier))
      .filter((battleGoal) => battleGoal)
      .map((battleGoal) => battleGoal as BattleGoal);
  ...
  setup() {
    this.dialog
      ...
      .closed.subscribe({
        next: () => {
          if (
            gameManager.battleGoalManager.getBattleGoals().length <
            (gameManager.fhRules(true) || settingsManager.settings.battleGoalsFh ? 3 : 2)
          ) {
            this.close();
          } else {
            this.update();
