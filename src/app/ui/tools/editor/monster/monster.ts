...
export const newMonsterJson: string =
  '{"name": "new-monster", "thumbnail" : "", "edition": "", "deck": "", "boss": false, "flying" : false, "hidden":false, "count": 10, "baseStat" : {}, "stats": []}';
...
export class MonsterEditorComponent implements OnInit {
  ...
  monsterDataToJson() {
    ...
    const compactData: any = JSON.parse(JSON.stringify(this.monsterData));
    ...
    if (!stat) {
      ...
      if (
        ...
        this.monsterData.errors.push(
          new FigureError(FigureErrorType.stat, 'monster', this.monsterData.name, this.monsterData.edition, type, '' + level)
        );
      }
